-- Activate 2단계 (onsite check-in) from the start of event day (KST), not just 1 hour before start time
...
BEGIN
  ...
  -- Activate onsite check-in from the start of the event day (KST)
  v_effective_start_ts := (v_date::text || ' 00:00')::timestamp AT TIME ZONE 'Asia/Seoul';
...
-- Onsite assertions: also require event day to have started (KST)
...
BEGIN
  ...
  v_day_start := (v_e.event_date::text || ' 00:00')::timestamp AT TIME ZONE 'Asia/Seoul';
  v_end := (v_e.event_date || ' ' || v_e.end_time)::timestamptz;
  IF now() < v_day_start THEN
    RAISE EXCEPTION 'Onsite not yet open' USING ERRCODE = 'P0004';
  END IF;
...
BEGIN
  ...
  v_day_start := (v_t.event_date::text || ' 00:00')::timestamp AT TIME ZONE 'Asia/Seoul';
