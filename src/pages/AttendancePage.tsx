...
const AttendancePage = () => {
  ...
  useEffect(() => {
    ...
    // Lock body scroll on signature screens to prevent iOS overscroll/address-bar jitter
    ...
    <div className="h-svh bg-muted/30 flex flex-col overflow-hidden" translate="no">
      ...
      <div className="flex-1 min-h-0 px-3 py-2 max-w-lg w-full mx-auto flex flex-col gap-2">
        <div className="bg-card rounded-lg shadow-card px-3 py-2 shrink-0">
          <h1 className="text-sm font-bold text-foreground leading-snug line-clamp-2">{event.title}</h1>
          <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1"><Calendar className="w-3 h-3 text-primary" />{event.event_date}</span>
            <span className="inline-flex items-center gap-1"><Clock className="w-3 h-3 text-primary" />{event.start_time?.slice(0,5)}~{event.end_time?.slice(0,5)}</span>
            <span className="inline-flex items-center gap-1"><MapPin className="w-3 h-3 text-primary" />{event.location}</span>
          </div>
        </div>
